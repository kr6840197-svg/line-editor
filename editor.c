...
/* ==========================================
 * BONUS FEATURE: Find and Replace
 * Syntax: r <old_word> <new_word>
 * ========================================== */
void replace_text(Document *doc, const char *old_word, const char *new_word) {
    if (doc->count == 0) {
    ...
    for (int i = 0; i < doc->count; i++) {
        char buffer[MAX_LEN] = "";
        char *src = doc->lines[i];
        char *match;

        while ((match = strstr(src, old_word)) != NULL) {
            // Copy everything up to the match
            strncat(buffer, src, match - src);
            // Append the replacement word
            strcat(buffer, new_word);
            // Move pointer past the replaced word
            src = match + old_len;
            total_replacements++;
        }
        ...
        strncpy(doc->lines[i], buffer, MAX_LEN - 1);
        doc->lines[i][MAX_LEN - 1] = '\0';
    }

    if (total_replacements == 0) {
        printf("Pattern '%s' not found in document.\n", old_word);
    } else {
        printf("Replaced %d occurrence(s) across document.\n", total_replacements);
    }
}
...
int main(void) {
    ...
    while (1) {
        ...
        // Replace: r <old> <new>
        else if (input[0] == 'r' && (input[1] == ' ' || input[1] == '\0')) {
            char old_word[64], new_word[64];
            if (sscanf(input, "r %63s %63s", old_word, new_word) == 2) {
                replace_text(&doc, old_word, new_word);
            } else {
                printf("Usage: r <old_word> <new_word>\n");
            }
        }
        // Unknown command
        else {
